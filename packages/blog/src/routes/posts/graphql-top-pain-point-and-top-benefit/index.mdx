...
excerpt: It's time to reconsider our choice of GraphQL if we're not reaping its ultimate advantage while enduring its primary drawback. Discover how to enhance the error handling interface and adopt the fragment masking pattern to truly harness the power of GraphQL in your projects.
date: 2023-03-12
tags: [graphql, error-handling, codegen]
featured: /posts/graphql-top-pain-point-and-top-benefit/cover.webp
---
...
<Image
  imgSrc="/posts/graphql-top-pain-point-and-top-benefit/pain-points.webp"
  imgAlt="pain points"
/>

<Image
  imgSrc="/posts/graphql-top-pain-point-and-top-benefit/strong-points.webp"
  imgAlt="strong points"
...
#### The Pitfalls of Using graphql-anywhere.filter() for Fragment Masking Pattern

Using the `filter` function in this context can lead to a problematic outcome. Let's break down the reasons using a simple example: Imagine you have an original dataset of size N getting from a GraphQL request with D levels and P properties for each level, and the component tree is a simple L-layer binary tree.

When you apply the `filter` function, it triggers a copying process that can repeat itself `(2^L) * (P^D)` times, ultimately creating `2^L` new objects. Considering that each of these objects can be as large as the original dataset (N), you're effectively navigating through this expanded landscape `2^L` times. This level of repetition can significantly impact performance and memory on the client.

To illustrate this further, take a look at the provided code snippet. The `executeSelectionSet` function is called (2^L) times from every component. On each call, there's an additional sequence of Deep copy operations (P^D) through DFS. This compounds the issue, making it apparent why using the `filter` function within the fragment masking pattern isn't recommended.

```ts
// In the following function, it gets invoked (2^L) times from each component.
// And with each invocation, it initiates another (P^D) set of Deep copy operations via DFS.
...
You can reference the original code here: [graphql-anywhere on npm](https://www.npmjs.com/package/graphql-anywhere?activeTab=code)
